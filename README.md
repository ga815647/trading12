...
目前策略模板與多維度矩陣涵蓋（**共 14 個 Trigger，21 個 Filter，總空間 > 10,000 個假設**）：

| 類別 | 代碼 | 群組 (Group) | 邏輯說明 |
|---|---|---|---|
| 籌碼集中 | A01, A03 | `chip` | 三大法人連續淨買量、集中度 |
| 動能 | B02, B03 | `momentum` | 連續長紅/長黑 K 棒、收盤漲幅 |
| 情緒 | J06 | `sentiment` | 融資餘額絕對量臨界點觸發 |
| 籌碼序列 | K01, K02, K03 | `chip_sequence` | 外資/投信/法人連續 N 日買超序列 |
| 跨資料源複合 | L01 | `composite` | 籌碼持有天數 + 股價連漲複合條件 |
| 群體行為背離 | M01, M02, M03 | `group_behavior` | 外資/投信/法人買且融資退（對立疊加）|

### 進階狀態分層過濾器 (State Filters)

系統在產生矩陣假設時，所有策略都會被疊加以下環境濾網（**21 個 Filter 維度**），以測試不同市場情境下的勝率差異：

**1. T/V/A 八狀態系統（8 個原子狀態 + 2 個複合狀態）**

| 代碼 | 狀態 | 意義 | 適合策略方向 |
|---|---|---|---|
| TVA1 | T+, V+, A+ | 全面多頭加速 | 追漲買進 |
| TVA2 | T+, V+, A- | 多頭但加速轉弱 | 觀察獲利了結 |
| TVA3 | T+, V-, A+ | 多頭中回調 | 等待確認再進 |
| TVA4 | T+, V-, A- | 多頭全面轉弱 | 賣出警戒 |
| TVA5 | T-, V+, A+ | 空頭中強力反彈 | 短線逆勢 |
| TVA6 | T-, V+, A- | 空頭中反彈趨緩 | 保守觀望 |
| TVA7 | T-, V-, A+ | 空頭但加速度回升 | **底部前兆**，逆勢布局 |
| TVA8 | T-, V-, A- | 全面崩跌 | 空頭機會 / 避免做多 |
| TVA_BULL | States 1+2 | 確定多頭區間 | 主力買進視窗 |
| TVA_REVERSAL | States 5+7 | 反轉前夕 | 底部買進機會 |

**2. 情緒臨界點分層**：利用 `SentimentLayerSystem` 捕捉恐慌、追高或聰明錢進場的關鍵轉折點。
...
每日掃描時，`vote_signals()` 跨越 **7 個獨立群組** (`chip`, `momentum`, `sentiment`, `chip_sequence`, `composite`, `group_behavior`, `price_volume`) 進行投票：
- ⭐ 1 顆星：1 個群組觸發（雜訊，不推播）
- ⭐⭐ 2 顆星：2 個獨立群組同時觸發（基礎買進候選）
- ⭐⭐⭐ 3 顆星：3+ 個獨立群組疊加（高信心做多訊號）

注意：`D` 類、`I` 類以及 `F02` 目前回測不支援 (`supported = False`)。
