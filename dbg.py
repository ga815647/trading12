...
for item in data:
    ...
    if item.get('sample_count', 0) < 200: failure_counts['sample_count'] += 1
    if item.get('win_rate', 0.0) < 0.55: failure_counts['win_rate'] += 1
    if item.get('oos_win_rate', 0.0) < 0.53: failure_counts['oos_win_rate'] += 1
    if item.get('sharpe', 0.0) < 1.0: failure_counts['sharpe'] += 1
    if float(item.get('p_value', 1.0)) >= 0.05: failure_counts['p_value_raw'] += 1
